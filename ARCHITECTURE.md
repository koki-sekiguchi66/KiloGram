...
| 学食メニュー | 3食堂の学食サイトから週次でメニューと栄養値を自動取得。食堂を選んで記録できる |
...
CafeteriaMenu     学食メニュー（3食堂ぶんをスクレイピングで週次更新）
                  一意キーは (cafeteria, menu_id)。同じ料理を複数の食堂が出すため
```
...
GitHub Actions（cron）→ SSH → python manage.py update_cafeteria_menus
   → 食堂ごとに menu.php の見出しを読み、その食堂が実際に持つ区分を得る
   → CafeteriaScraper が区分別に巡回
      （リクエスト間に待機を入れ、相手サーバーに負荷をかけない）
      （栄養値は menu_id ごとに1回だけ引き、食堂をまたぐ重複を再取得しない）
   → 既存の CafeteriaMenu を全削除し bulk_create で入れ替え
   → 失敗時は非ゼロ終了し、GitHub Actions 側で失敗として検知される
```

区分は食堂ごとに違うため、共通の5区分だけをコード化し、それ以外はサイトの見出しを
`category_label` に保存する（→ [decisions.md #31](docs-public/decisions.md)）。
