...
**実装はドキュメントまで含めて完了。** コードとテストが通っただけで終わりにしない。
特に、機能を足したら**「未実装」「できない」と書いていた記述が嘘になっていないか**を確認する
（`docs-public/` の「今後の課題」、過去の ADR、MCP ツールの description）。手順は `write-docs` Skill。

置き場所は読者で決める。**public 公開リポジトリ**であることに注意する。
